...
class Ui_Form(QMainWindow,object):
    def search_(self):
        students = session.query(classteachers).filter_by(name=self.search.text().upper()).all()
        if len(students)>0:
            for i, student in enumerate(students):
                    self.tableWidget.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(str(student.id)))
                    self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(student.name))
                    self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(student.classroom))
...
"background-color: rgb(255, 85, 0);")
    ...
    def retranslateUi(self, Form):
        ...
        from models import classteachers
        students = session.query(classteachers).all()
