...
# Form implementation generated from reading ui file 'students_dashboard.ui'
#
...
from models import Students,Parents,session,fee_payed
import dialog
from PyQt5.QtWidgets import QApplication, QMainWindow
class Ui_Form(QMainWindow,object):
    def search_(self):
        students = session.query(Students).filter_by(full_name=self.search.text().upper()).all()
        if len(students)>0:
            for i, student in enumerate(students):
                self.tableWidget.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(str(student.id)))
                self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(str(student.index_no)))
                self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(student.full_name))
                self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(student.classroom))
                self.tableWidget.setItem(i, 3, QtWidgets.QTableWidgetItem(student.guardian))
                self.tableWidget.setItem(i, 4, QtWidgets.QTableWidgetItem(student.dob))
                self.tableWidget.setItem(i, 5, QtWidgets.QTableWidgetItem(student.admission_date))
                ...
                ui_dialog.more_info_1 = '''You may have entered only one name,You may have mispelled the name,You entered a name without a space.eg,onesmusbett instead of onesmus bett
                '''
    ...
    def setupUi(self, Form):
        ...
        for student in students:
            self.tableWidget.setVerticalHeaderItem(student.id-1, item)
            item = QtWidgets.QTableWidgetItem()
            font = QtGui.QFont()
            font.setBold(True)
            font.setWeight(75)
...
"background-color: rgb(255, 85, 0);")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "Students Database"))
        students=session.query(Students).all()
        for i in students:
            item = self.tableWidget.verticalHeaderItem(i.id-1)
            item.setText(_translate("Form", str(i.id)))
        item = self.tableWidget.horizontalHeaderItem(0)
        item.setText(_translate("Form", "Adm no"))
        item = self.tableWidget.horizontalHeaderItem(1)
        item.setText(_translate("Form", "Full Name    "))
        item = self.tableWidget.horizontalHeaderItem(2)
        item.setText(_translate("Form", "Class room"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("Form", "Guardian"))
        item = self.tableWidget.horizontalHeaderItem(4)
        item.setText(_translate("Form", "Date of Birth"))
        item = self.tableWidget.horizontalHeaderItem(5)
        item.setText(_translate("Form", "Admission Date"))
