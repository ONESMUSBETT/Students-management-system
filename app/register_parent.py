...
# Form implementation generated from reading ui file 'register_parent.ui'
#
...
from models import Parents,session,Students
from dialog import Ui_Dialog
from PyQt5.QtWidgets import QApplication, QMainWindow
class Ui_MainWindow(QMainWindow,object):
    def submit_(self):
        if self.full_name.text() == '' or self.adm_no.text() == '' or self.contacts.text() == '' or self.location.text() == '':
                Dialog = QtWidgets.QDialog()
                ui_dialog = Ui_Dialog()
                ui_dialog.win_title='Error'
                ui_dialog.title='VALUE CANNOT BE NULL'
                ui_dialog.message_head='This error occured because some values seem to be empty!\n Please try again!!!'
                ui_dialog.setupUi(Dialog)
                Dialog.show()
                Dialog.exec_()       
        else:
                parent = session.query(Parents).filter_by(full_name=self.full_name.text().upper(),children=self.adm_no.text().upper(),location=self.location.text().upper()).first()
                if parent:
                        Dialog = QtWidgets.QDialog()
                        ui_dialog = Ui_Dialog()
                        ui_dialog.win_title='Error'
                        ui_dialog.title=f'Parent {parent.full_name} is already Registered '
                        ui_dialog.message_head='The parent with the following info is in database:'
                        ui_dialog.more_info_1=f'adm_no: {parent.adm_no}'
                        ui_dialog.more_info_2=f"Location: {parent.location}"
                        ui_dialog.setupUi(Dialog)
                        Dialog.show()
                        Dialog.exec_()    
                        session.commit()
                      ...
                      new_parent = Parents(full_name=self.full_name.text().upper(),children=int(self.adm_no.text().upper()),contacts=self.contacts.text().upper(),location=self.location.text().upper(),childID=student.id)
...
"font: 87 12pt \"Segoe UI Black\";")
        ...
        self.contacts.setGeometry(QtCore.QRect(50, 500, 711, 71))
...
"font: 11pt \"MS Shell Dlg 2\";")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Register Parent"))
        self.label_2.setText(_translate("MainWindow", "REGISTER PARENT"))
        self.label_3.setText(_translate("MainWindow", "Full Name:"))
        self.label_4.setText(_translate("MainWindow", "Adm no:"))
