...
# Form implementation generated from reading ui file 'parents_dashboard.ui'
#
...
from models import Parents,Students,session,fee_payed
import dialog
from PyQt5.QtWidgets import QApplication, QMainWindow
class Ui_Form(QMainWindow,object):
    def search_(self):
        parents = session.query(fee_payed).filter_by(adm=int(self.search.text())).all()
        if len(parents)>0:
            for i, student in enumerate(parents):
                self.tableWidget.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(str(student.id)))
                self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(str(student.adm)))
                self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(str(student.bal)))
                self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(str(student.amount)))
                self.tableWidget.setItem(i, 3, QtWidgets.QTableWidgetItem(str(student.date)))
        else:
                Dialog = QtWidgets.QDialog()
                ui_dialog = dialog.Ui_Dialog()
                ui_dialog.win_title='Error'
                ui_dialog.title_='Admission SEARCH ERROR'
                ui_dialog.message_head='This is because: '
                ui_dialog.more_info_1 = '''You may have entered a name, You should only enter admission numbers!
                '''
...
"background-color: rgb(255, 85, 0);")
    ...
    def retranslateUi(self, Form):
        ...
        item.setText(_translate("Form", "Adm"))
        ...
        from models import fee_payed
        parents = session.query(fee_payed).all()
        self.tableWidget.setRowCount(len(parents))
        for i, parent in enumerate(parents):
            self.tableWidget.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(str(parent.id)))
            self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(str(parent.adm)))
            self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(str(parent.bal)))
            self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(str(parent.amount)))
            self.tableWidget.setItem(i, 3, QtWidgets.QTableWidgetItem(str(parent.date)))
            
        self.search.setPlaceholderText(_translate("Form", "Search adm no..."))
