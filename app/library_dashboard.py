...
# Form implementation generated from reading ui file 'library_dashboard.ui'
#
...
from models import session,Library,bookTaken
from PyQt5.QtWidgets import QApplication, QMainWindow
class Ui_Form(QMainWindow,object):
    def search_(self):
        if self.search.text() == '':
            parents = session.query(bookTaken).all()
            ...
            for i, parent in enumerate(parents):
                ...
                self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(str(parent.adm)))
                self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(parent.status))
                self.tableWidget.setItem(i, 3, QtWidgets.QTableWidgetItem(parent.title))
        else:
            student = session.query(bookTaken).filter_by(adm=int(self.search.text().upper())).all()
            if student:
                self.tableWidget.setRowCount(len(student))
                for i, parent in enumerate(student):
                    self.tableWidget.setVerticalHeaderItem(i, QtWidgets.QTableWidgetItem(str(parent.id)))
                    self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(parent.name))
                    self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(str(parent.adm)))
                    self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(parent.status))
...
"background-color: rgb(255, 85, 0);")
    ...
    def retranslateUi(self, Form):
        ...
        self.search.setPlaceholderText(_translate("Form", "Search admission number..."))
