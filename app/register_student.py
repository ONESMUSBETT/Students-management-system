...
# Form implementation generated from reading ui file 'register_student.ui'
#
...
from models import Classroom,Teachers,Students,Subjects,Parents,session,fee_payed,Fees
...
class Ui_MainWindow(QMainWindow,object):
    ...
    def register(self):
        ...
        if student:
                ...
                student = session.query(Students).filter_by(index_no=self.adm_no.text()).first()
                ui_dialog.title=f'STUDENT {student.full_name} ALREADY EXISTS'
                ui_dialog.info='Name:'
                ui_dialog.info_2=student.full_name
                ui_dialog.message_head='The student you are trying to enter into the database already exists!'
                ui_dialog.more_info_1=f"Admision Number: {student.index_no}"
                ui_dialog.more_info_2=f'Class Room: {student.classroom}'
                ui_dialog.setupUi(Dialog)
                Dialog.show()
                Dialog.exec_()       
        else:
            b = session.query(Fees).filter_by(id=1).first()
            total = b.total_term1+b.total_term2+b.total_term3
            data = fee_payed(name=self.full_name.text().upper(),adm=int(self.adm_no.text()),bal=total,amount=0)
            session.add(data)
            session.commit()
            new_student = Students(full_name=self.full_name.text().upper(),index_no=int(self.adm_no.text()),guardian=self.guardian.text().upper(),dob=self.dob.text(),classroom=self.comboBox.currentText())
            session.add(new_student)
            session.commit()
            s = session.query(Students).filter_by(index_no=self.adm_no.text())
            for i in s:
                #uSE A QDIALOG BOX           
                Dialog = QtWidgets.QDialog()
                ui_dialog = Ui_Dialog()
                ui_dialog.win_title = 'Success'
                ui_dialog.title = f'USER {i.full_name} HAS BEEN REGISTERED'
...
"font: 11pt \"MS Shell Dlg 2\";")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_2.setText(_translate("MainWindow", "REGISTER STUDENT"))
